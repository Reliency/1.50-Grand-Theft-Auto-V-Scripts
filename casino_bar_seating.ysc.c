...
int func_66()
{
	...
	if (func_80(PLAYER_ID()) && PLAYER_ID())
	{
		return 1;
	}
	if (func_69())
	...
	else
	{
		...
		if (NETWORK_IS_PLAYER_ACTIVE(iParam0) && (NETWORK_IS_PLAYER_ACTIVE(iParam0) || iParam3 == 1))
		{
			...
			if (!bVar19)
			{
				...
				else if (bVar13 || (!func_97(PLAYER_ID(), 0) && !func_96()))
				{
			...
			if (bParam1)
			{
				...
				if (!func_92(iVar26) && !IS_ENTITY_ATTACHED_TO_ANY_VEHICLE(iVar26))
				{
					if (!bVar21)
					{
