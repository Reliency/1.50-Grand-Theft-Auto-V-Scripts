			...
			if (DOES_ENTITY_EXIST(DOES_ENTITY_EXIST()))
			{
				if (!IS_PED_INJURED(IS_PED_INJURED()))
				{
					if ((!IS_PED_FALLING(IS_PED_FALLING()) && !IS_PED_FALLING(IS_PED_FALLING())) && !IS_PED_FALLING(IS_PED_FALLING()))
					{
						if (!bVar0)
						{
							if (!((func_28() || IS_PLAYER_SWITCH_IN_PROGRESS()) || func_27()))
							{
								if (func_13())
								{
									bVar0 = true;
						...
						else
						{
							if (iLocal_28)
							{
								SET_PLAYER_CONTROL(SET_PLAYER_CONTROL(), false, 384);
...
void func_39(int iParam0)
{
	...
	Global_41948[iParam0] = GET_GAME_TIMER();
}

void func_40()
{
	Global_41947 = GET_GAME_TIMER();
}
