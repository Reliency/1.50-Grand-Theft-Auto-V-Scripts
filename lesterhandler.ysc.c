...
void __EntryFunction__()
{
	...
	while ((!HAS_ANIM_DICT_LOADED("MISSHEIST_JEWEL@HACKING") || !HAS_ANIM_DICT_LOADED(joaat("prop_cs_walking_stick"))) || !HAS_ANIM_DICT_LOADED(joaat("prop_laptop_lester2")))
	{
		wait(0);
	}
	...
	iLocal_218 = GET_NAMED_RENDERTARGET_RENDER_ID("tvscreen");
	while (!IS_BIT_SET(iLocal_216, 1))
	{
		if (!IS_BIT_SET(iLocal_216, 0))
		{
			if (DOES_ENTITY_EXIST(iLocal_46))
			{
				if (!IS_PED_INJURED(iLocal_46))
				{
					...
					SET_PED_CAN_EVASIVE_DIVE(iLocal_46, 0);
					SET_ENTITY_IS_TARGET_PRIORITY(iLocal_46, 0, 0);
					SET_PED_CAN_RAGDOLL(iLocal_46, 0);
					SET_PED_ID_RANGE(iLocal_46, 250f);
					...
					TASK_PLAY_ANIM_ADVANCED(0, "MISSHEIST_JEWEL@HACKING", "HACK_INTRO", vVar0, vVar1, 8f, -8f, -1, 262152, 0, 2, 0);
					TASK_PLAY_ANIM_ADVANCED(0, "MISSHEIST_JEWEL@HACKING", "HACK_LOOP", vVar0, vVar1, 8f, -8f, -1, 262153, 0, 2, 0);
					...
					iLocal_47 = CREATE_OBJECT(joaat("prop_cs_walking_stick"), 706.9874f, -967.0945f, 30.4f, false, false, false);
					...
					iLocal_48 = CREATE_OBJECT(joaat("prop_laptop_lester2"), 707.3041f, -967.6456f, 30.376f, true, true, false);
		...
		else if (!IS_ENTITY_DEAD(IS_ENTITY_DEAD(), 0))
		{
			...
			{
				fVar2 = vdist2(GET_ENTITY_COORDS(GET_ENTITY_COORDS(), true), GET_ENTITY_COORDS(iLocal_46, true));
				if (func_22() == 0)
				{
					if (fVar2 < 4f)
					{
				...
				if (Global_76841 == 86)
				{
					if (!Global_111555 == 1 && IS_ENTITY_IN_ANGLED_AREA(IS_ENTITY_IN_ANGLED_AREA(), 718.0749f, -976.0472f, 22.91482f, 718.0593f, -979.455f, 27.1191f, 5.75f, 0, true, 0))
					{
						func_3(7, 1);
						Global_111551 = 0;
						Global_111555 = 1;
...
void func_1()
{
	if (func_2())
	{
		...
		{
			SET_TEXT_RENDER_ID(iLocal_218);
			DRAW_SCALEFORM_MOVIE(iLocal_219, 0.08f, 0.17f, 0.27f, 0.45f, 255, 255, 255, 255, 0);
...
int func_2()
{
	iLocal_219 = 0x67D02A194A2FC2BD("JHPB_02_Laptop");
	if (HAS_SCALEFORM_MOVIE_LOADED(iLocal_219))
	{
		return 1;
	}
	return 0;
...
void func_3(int iParam0, bool bParam1)
{
	...
	if (iVar0 < 31)
	{
		if (bParam1)
		{
			SET_BIT(&(Global_111560.f_24990), iVar0);
		}
		else
		{
			CLEAR_BIT(&(Global_111560.f_24990), iVar0);
		}
	}
	else
	{
		...
		if (bParam1)
		{
			SET_BIT(&(Global_111560.f_24990.f_1), iVar0);
		}
		else
		{
			CLEAR_BIT(&(Global_111560.f_24990.f_1), iVar0);
		}
	}
}
...
void func_30()
{
	SET_PED_AS_NO_LONGER_NEEDED(&iLocal_46);
	SET_MODEL_AS_NO_LONGER_NEEDED(joaat("prop_cs_walking_stick"));
	SET_OBJECT_AS_NO_LONGER_NEEDED(&iLocal_47);
	SET_MODEL_AS_NO_LONGER_NEEDED(joaat("prop_cs_walking_stick"));
	SET_OBJECT_AS_NO_LONGER_NEEDED(&iLocal_48);
	SET_MODEL_AS_NO_LONGER_NEEDED(joaat("prop_laptop_lester2"));
...
{
	func_32();
	SET_SCALEFORM_MOVIE_AS_NO_LONGER_NEEDED(&iLocal_219);
}
