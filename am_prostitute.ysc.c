...
void __EntryFunction__()
{
	...
	{
		func_45();
		if (((func_34() || NETWORK_END_TUTORIAL_SESSION()) || (func_33(NETWORK_END_TUTORIAL_SESSION()) || func_32(NETWORK_END_TUTORIAL_SESSION()))) || func_29(NETWORK_END_TUTORIAL_SESSION()))
		{
			func_46();
		}
		if (NETWORK_IS_GAME_IN_PROGRESS())
		...
		if ((iLocal_54 % iVar0) == 0)
		{
			if (NETWORK_IS_PARTICIPANT_ACTIVE(NETWORK_IS_PARTICIPANT_ACTIVE(iVar0)))
			{
				...
				if (!IS_ENTITY_DEAD(iVar2, 0))
				{
					...
					{
						if (vdist2(GET_ENTITY_COORDS(GET_ENTITY_COORDS(), false), GET_ENTITY_COORDS(iVar2, true)) <= 22500f)
						{
							if (!iLocal_56)
							{
...
void func_2()
{
	...
	switch (iLocal_52)
	{
		...
		case 3:
			if ((HAS_SCRIPT_LOADED("pb_prostitute") && HAS_SCRIPT_LOADED(joaat("pb_prostitute")) <= 0) && !HAS_SCRIPT_LOADED("pb_prostitute", HAS_SCRIPT_LOADED(), 1, 0))
			{
				iVar4 = func_12(iVar2);
...
int func_4()
{
	if (_GET_NUMBER_OF_INSTANCES_OF_SCRIPT_WITH_NAME_HASH(joaat("pb_prostitute")) <= 0 && !_GET_NUMBER_OF_INSTANCES_OF_SCRIPT_WITH_NAME_HASH("pb_prostitute", _GET_NUMBER_OF_INSTANCES_OF_SCRIPT_WITH_NAME_HASH(), 1, 0))
	{
		return 1;
	}
	if (iLocal_57 == 0)
...
void func_5()
{
	...
	if (iVar1 != 0)
	{
		TRIGGER_SCRIPT_EVENT(1, &Var0, 2, iVar1);
	}
}
		...
		if (DOES_ENTITY_EXIST(DOES_ENTITY_EXIST()))
		{
			if (IS_PED_IN_ANY_VEHICLE(IS_PED_IN_ANY_VEHICLE(), 0))
			{
				...
				if (func_28(&iVar2))
				{
					...
					{
						if (func_12(iVar1) == GET_PED_IN_VEHICLE_SEAT(iVar3, 0, 0) || GET_PED_IN_VEHICLE_SEAT(func_12(iVar1), iVar3, 1))
						{
					...
					vVar4 = { GET_ENTITY_COORDS(func_12(iVar1), false) };
					vVar5 = { GET_ENTITY_COORDS(iVar0, false) };
					vVar6 = { GET_ENTITY_COORDS(GET_ENTITY_COORDS(), false) };
...
int func_14()
{
	...
	iVar0 = 0;
	iVar2 = GET_PED_NEARBY_PEDS(GET_PED_NEARBY_PEDS(), &uVar1, -1);
	if (iVar2 > 0)
	{
		iVar3 = 0;
		while (iVar3 <= 7)
...
void func_46()
{
	...
	if (!func_50(PLAYER_ID()) && !func_49(PLAYER_ID()))
	{
		func_48();
	}
	func_4();
