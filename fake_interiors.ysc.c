...
void __EntryFunction__()
{
	...
	while (true)
	{
		...
		if (func_23(PLAYER_ID()) && PLAYER_ID(func_22()))
		{
			iLocal_31 = func_22();
		}
		else
...
void func_1()
{
	...
	{
		vVar0 = { GET_ENTITY_COORDS(iLocal_31, false) };
		...
		if (vdist2(vVar1, vVar0) < IntToFloat(func_18(iLocal_29)))
		{
			...
			while (iVar3 < func_17(iLocal_29))
			{
				if (!bVar2)
				{
					if (IS_ENTITY_IN_ANGLED_AREA(iLocal_31, func_16(iLocal_29, iVar3), func_15(iLocal_29, iVar3), func_14(iLocal_29, iVar3), 0, true, 0))
					{
						bVar2 = true;
					}
				}
			...
			if (bVar2)
			{
				SET_RADAR_AS_INTERIOR_THIS_FRAME(SET_RADAR_AS_INTERIOR_THIS_FRAME(func_13(iLocal_29)), vVar1.x, vVar1.y, func_12(iLocal_29), func_11(iLocal_29));
				func_10(iLocal_29);
			}
		}
	}
